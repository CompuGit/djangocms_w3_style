...
CLASS_CHOICES = getattr(
    settings,
    'DJANGOCMS_W3_STYLE_CHOICES',
    ['w3-container', 'w3-display-container', 'w3-panel', 'w3-block', 'w3-row', 'w3-row-padding', 'w3-col', 'w3-btn', 'w3-button', 'w3-image', 'w3-input'],
)
CLASS_CHOICES = tuple((entry, entry) for entry in CLASS_CHOICES)
TAG_CHOICES = getattr(
    settings,
    'DJANGOCMS_W3_STYLE_TAGS',
...
def get_templates():
    ...
    choices += getattr(
        settings,
        'DJANGOCMS_W3_STYLE_TEMPLATES',
        [],
    )
    return choices


class W3_Style(CMSPlugin):
    """
